...
| Engine capability routing | Unity is the only active engine capability surface. | Unity-first capability routing with Ouroforge named as the next product priority but not a runtime surface yet. | Removed engine claims can drift back into docs or tests. | High | Keep Godot and Three.js absent from runtime projections, CLI, bridge assets, and roadmap templates. |
...
- **Status**: Contract implemented — `gateway/src/lux_game_context.rs` defines the Unity-first Game Context Adapter observation schema; missing observations must write blocker evidence.
...
- `bridge/` is the in-repository Unity bridge source area.
...
| Bridge source | In-repository Unity source area | `bridge/` | Fresh checkouts must make the Unity bridge source availability explicit before bridge install can be considered verified. |
...
- **Capability routing**: Unity is the only active LUX engine surface. Godot and Three.js must remain absent unless a future accepted roadmap decision restores them.
...
| `bridge/` | **Continue** as in-repository Unity bridge source. | The Unity bridge source lives under `bridge/unity/` and installs to `Assets/Editor/LuxBridge/`, matching the documented Unity target layout. | Restore `Assets/Editor/AiBridgeEditor/` only if a supported Unity install surface requires that exact path. |
